"""
Тестовый скрипт для проверки BANG с визуализацией
...
def test_basic_2d_clustering():
    ...
    config = StrategiesManager.getStrategyRunConfigById("bang")
    config["levels"] = 15
    strategy = ConcreteStrategyBANG()
...
def test_parameter_sensitivity():
    print("="*80)
    print("ТЕСТ 2: Чувствительность к Параметрам (levels)")
    print("="*80)
    if not ALGORITHM_AVAILABLE:
        print("❌ Алгоритм не доступен. Пропускаем тест.")
        return
    X, y_true = generate_test_data_2d('blobs', n_samples=200, centers=3)
    levels_values = [10, 15, 20, 25]
    fig, axes = plt.subplots(2, 2, figsize=(14, 14))
    axes = axes.ravel()
    for i, level_val in enumerate(levels_values):
        config = StrategiesManager.getStrategyRunConfigById("bang")
        config["levels"] = level_val
        strategy = ConcreteStrategyBANG()
        y_pred = strategy.clastering_points(X, config)
        n_clusters = len(np.unique(y_pred))
        axes[i].scatter(X[:, 0], X[:, 1], c=y_pred, cmap='tab10', alpha=0.6, s=30, edgecolors='k', linewidth=0.5)
        axes[i].set_title(f'levels={level_val}\n({n_clusters} кластеров)', fontsize=12, fontweight='bold')
        axes[i].set_xlabel('X₁')
        axes[i].set_ylabel('X₂')
        axes[i].grid(True, alpha=0.3)
        print(f"  levels={level_val}: {n_clusters} кластеров")
    plt.tight_layout()
