"""
Тестовый скрипт для проверки HDBSCAN с визуализацией
...
try:
    from ClusteringMethods.ClasteringAlgorithms import (
        ConcreteStrategyHDBSCAN_from_SKLEARN,
        Context,
        StrategiesManager
    )
...
def test_basic_2d_clustering():
    ...
    config = StrategiesManager.getStrategyRunConfigById("hdbscan_sk")
    config["min_cluster_size"] = 10
    config["min_samples"] = 5
    strategy = ConcreteStrategyHDBSCAN_from_SKLEARN()
...
def test_parameter_sensitivity():
    print("="*80)
    print("ТЕСТ 2: Чувствительность к Параметрам (min_cluster_size)")
    print("="*80)
    if not ALGORITHM_AVAILABLE:
        print("❌ Алгоритм не доступен. Пропускаем тест.")
        return
    ...
    for i, min_size in enumerate(min_cluster_sizes):
        config = StrategiesManager.getStrategyRunConfigById("hdbscan_sk")
        config["min_cluster_size"] = min_size
        config["min_samples"] = 5
        strategy = ConcreteStrategyHDBSCAN_from_SKLEARN()
        y_pred = strategy.clastering_points(X, config)
        ...
        axes[i].set_title(f'min_cluster_size={min_size}\n({n_clusters} кластеров, {n_noise} шум)', fontsize=12, fontweight='bold')
        axes[i].set_xlabel('X₁')
        axes[i].set_ylabel('X₂')
        axes[i].grid(True, alpha=0.3)
        print(f"  min_cluster_size={min_size}: {n_clusters} кластеров, {n_noise} шум")
    plt.tight_layout()
