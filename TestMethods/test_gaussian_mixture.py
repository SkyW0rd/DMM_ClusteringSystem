"""
Тестовый скрипт для проверки Gaussian Mixture с визуализацией
...
try:
    from ClusteringMethods.ClasteringAlgorithms import (
        ConcreteStrategyGaussianMixture_from_SKLEARN,
        Context,
        StrategiesManager
    )
...
def test_basic_2d_clustering():
    ...
    config = StrategiesManager.getStrategyRunConfigById("gaussian_mixture_sk")
    config["n_components"] = 3
    strategy = ConcreteStrategyGaussianMixture_from_SKLEARN()
...
def test_parameter_sensitivity():
    print("="*80)
    print("ТЕСТ 2: Чувствительность к Параметрам (n_components)")
    print("="*80)
    if not ALGORITHM_AVAILABLE:
        print("❌ Алгоритм не доступен. Пропускаем тест.")
        return
    ...
    for i, n_comp in enumerate(n_components_values):
        ...
        axes[i].set_title(f'n_components={n_comp}\n({n_clusters} кластеров)', fontsize=12, fontweight='bold')
        axes[i].set_xlabel('X₁')
        axes[i].set_ylabel('X₂')
        axes[i].grid(True, alpha=0.3)
        print(f"  n_components={n_comp}: {n_clusters} кластеров")
    plt.tight_layout()
