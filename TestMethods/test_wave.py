...
def generate_test_data_2d(dataset_type='blobs', n_samples=300, **kwargs):
    """
    Генерация данных для WaveClustering.
    WaveClustering работает с 2D данными.
    """
...
def generate_test_data_3d(n_samples=300, n_clusters=3, cluster_std=0.5):
    """
    Генерация данных для WaveClustering.
    WaveClustering работает с 2D, поэтому используем проекцию на первые 2 измерения.
    """
    X, y_true = make_blobs(
        n_samples=n_samples,
        n_features=3,
...
def test_basic_2d_clustering():
    ...
    n_clusters_pred = len(np.unique(y_pred[y_pred != 0]))  # 0 обычно означает шум
    n_clusters_true = len(np.unique(y_true))
    n_noise = np.sum(y_pred == 0)
    ...
    axes[1].set_title(f'{ALGORITHM_NAME} Результаты\n({n_clusters_pred} кластеров, {n_noise} шум)', fontsize=14, fontweight='bold')
...
def test_parameter_sensitivity():
    print("="*80)
    print("ТЕСТ 2: Чувствительность к Параметрам (density_threshold)")
    print("="*80)
    if not ALGORITHM_AVAILABLE:
        print("❌ Алгоритм не доступен. Пропускаем тест.")
        return
    X, y_true = generate_test_data_2d('blobs', n_samples=300, centers=3)
    density_threshold_values = [0.05, 0.10, 0.15, 0.20]
    fig, axes = plt.subplots(2, 2, figsize=(14, 14))
    axes = axes.ravel()
    for i, density_threshold in enumerate(density_threshold_values):
        config = StrategiesManager.getStrategyRunConfigById("waveclustering")
        config["n_grid"] = 32
        config["wavelet"] = "db4"
        config["n_levels"] = 2
        ...
        strategy = ConcreteStrategyWaveClustering()
        y_pred = strategy.clastering_points(X, config)
        n_clusters = len(np.unique(y_pred[y_pred != 0]))
        axes[i].scatter(X[:, 0], X[:, 1], c=y_pred, cmap='tab10', alpha=0.6, s=30, edgecolors='k', linewidth=0.5)
        axes[i].set_title(f'density_threshold={density_threshold}\n({n_clusters} кластеров)', fontsize=12, fontweight='bold')
        axes[i].set_xlabel('X₁')
        axes[i].set_ylabel('X₂')
        axes[i].grid(True, alpha=0.3)
        print(f"  density_threshold={density_threshold}: {n_clusters} кластеров")
    plt.tight_layout()
...
def test_3d_clustering():
    ...
    # WaveClustering работает с 2D, используем проекцию на первые 2 измерения
    X_2d = X[:, :2]
    config = StrategiesManager.getStrategyRunConfigById("waveclustering")
    config["n_grid"] = 32
    config["wavelet"] = "db4"
