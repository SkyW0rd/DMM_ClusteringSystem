"""
Тестовый скрипт для проверки OPTICS с визуализацией
...
try:
    from ClusteringMethods.ClasteringAlgorithms import (
        ConcreteStrategyOPTICS_from_SKLEARN,
        Context,
        StrategiesManager
    )
...
def test_basic_2d_clustering():
    ...
    config = StrategiesManager.getStrategyRunConfigById("optics_sk")
    config["min_samples"] = 5
    config["max_eps"] = float("inf")
    config["cluster_method"] = "xi"
    strategy = ConcreteStrategyOPTICS_from_SKLEARN()
...
def test_parameter_sensitivity():
    print("="*80)
    print("ТЕСТ 2: Чувствительность к Параметрам (min_samples)")
    print("="*80)
    if not ALGORITHM_AVAILABLE:
        print("❌ Алгоритм не доступен. Пропускаем тест.")
        return
    ...
    for i, min_samples_val in enumerate(min_samples_values):
        config = StrategiesManager.getStrategyRunConfigById("optics_sk")
        config["min_samples"] = min_samples_val
        config["max_eps"] = float("inf")
        config["cluster_method"] = "xi"
        strategy = ConcreteStrategyOPTICS_from_SKLEARN()
        y_pred = strategy.clastering_points(X, config)
        ...
        axes[i].set_title(f'min_samples={min_samples_val}\n({n_clusters} кластеров, {n_noise} шум)', fontsize=12, fontweight='bold')
        axes[i].set_xlabel('X₁')
        axes[i].set_ylabel('X₂')
        axes[i].grid(True, alpha=0.3)
        print(f"  min_samples={min_samples_val}: {n_clusters} кластеров, {n_noise} шум")
    plt.tight_layout()
